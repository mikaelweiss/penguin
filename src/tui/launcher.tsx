...
export function Launcher({
  ...
  const [entries, setEntries] = useState<workflows.Found[]>([]);
  ...
  useEffect(() => {
    let gone = false;
    void workflows.listed(process.cwd()).then((found) => {
  ...
  const begin = async (entry: workflows.Found): Promise<void> => {
    setWarn("");
...
function List({
  ...
  entries: workflows.Found[];
  at: number;
  width: number;
}): ReactNode {
  if (entries.length === 0) {
    const places = workflows.searched(process.cwd()).map(short).join(" or ");
