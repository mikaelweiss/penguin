...
- The binary carries the starter catalog, so `packages/engine/src/catalog/starter.generated.ts` is checked in. Run `bun run generate` after editing `packages/engine/examples/` or the version in the root `package.json`. A test fails when it is stale, and a build refuses to ship it.
