...
- `extended-ticket`: the full pipeline, built on the default `ticket`:
  ...
  - A split decision breaks a large item into smaller tickets and spawns one child run per ticket.
  - The plan ends in acceptance criteria that the reviewer judges verbatim.
  - Implement dispatches to a per-change-type skill and can fan work out to parallel child runs.
