...
- `install`: draw the penguin wordmark, create `~/.penguin/` and `~/.local/state/penguin/runs/`, copy the catalog (`30-defaults.md`) into `~/.penguin/`, which carries the `defaults` file with the role choices, write `penguin-env.d.ts`, then sync the global skills (skills above). The first penguin command runs it, except `intro`.
