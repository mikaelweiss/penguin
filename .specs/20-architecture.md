...
- **view**: `build(host)` returns `{render(event)}`. It writes the scroll and never the bottom of the screen: the viewer erases its own rows before each render and draws them again after. A state, fact, or watch event reaches the adapter only when no viewer draws a status line, because that line is where those belong. The terminal adapter reads markdown as markdown, wrapped to the width, dims thinking, and cuts a tool call to one line. With no view adapter installed the engine prints plain lines itself.
