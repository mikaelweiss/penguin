...
- **Viewer**: the full-screen terminal UI, one projection of the event files. Bare `pn` opens the dashboard: the live runs with their state, and a needs-you list of every open gate and credential ask across them. The runs heading carries what the machine has left, the memory in use out of the total and the load average out of the core count, so the count of runs to start is visible before one more starts. It turns amber when another run would oversubscribe the machine. `y` copies the directory of the line under the cursor. `d` reveals a done section under the live runs, every other run on disk with how it ended, and a second `d` hides it. Enter opens a done run read-only. The dashboard also starts a run: `n` lists every workflow, asks for the params it needs, and opens the new run's view. Opening a run shows its view: the run's tree on the left (activities nested under their parents, sessions under the activity that opened them, each with a state glyph), the selected node's transcript on the right with the input bar under it, and one status line across the bottom. `pn run` opens the view on the new run, and `pn attach` joins one later with its full history. Piped or in cron, penguin prints plain lines instead. Closing a viewer never touches the run.
...
- **agent**: `build(host)` returns `{turn(turn)}`. The engine owns sessions, skill resolution, prompt assembly, schema validation, and the retry rule. The adapter owns the CLI invocation and the result transport. Each turn receives the session id, whether the session has had a turn before, the cwd, the prompt, the JSON schema when a result is wanted, and the session options. When the workflow stops a turn, the engine kills the turn's process. The claude adapter runs `claude -p` with `--session-id` on the first turn and `--resume` after, reads stream-json into typed agent events, and takes results from `--json-schema` structured output. The codex adapter runs `codex exec` with the prompt on stdin, holds the codex thread id for each penguin session so a later turn runs `codex exec resume`, reads the `--json` stream into the same agent events, and takes results from an `--output-schema` file. The cursor adapter runs `cursor-agent -p --force`, keeps the chat id its stream reports for each session and passes it back with `--resume`, reads stream-json into the same agent events, and asks for the result as one JSON object in the reply text. The pi adapter runs `pi --mode json` with `--session-id` on every turn, reads the wire events into the same agent events, and takes results from a `penguin_result` tool it registers through a per-turn extension file. The opencode adapter runs `opencode run --format json` with the prompt on stdin, because a prompt opens with the skill's frontmatter and an argument parser reads that as options. opencode names its own session, so the adapter maps the handle onto the id its events carry, and it asks for a typed result in the prompt because the CLI takes no schema.
...
In a run view: the run's tree fills the left pane, the selected node's transcript fills the right, and the input bar sits under the transcript at the transcript's width. The tree derives from the events alone: activities nest under their parents, sessions sit under the activity that opened them, and each node shows a state glyph, running, blocked, idle, done, or failed, derived from the open steps, gates, and waits in its subtree with the same precedence the run state uses. With the input empty, arrows move through the tree, left and right fold a node, enter opens or closes it, and `q` goes to the dashboard. `y` copies the selected node's directory to the system clipboard. The directories are the ones of the sessions in the node's subtree, or the run's own folder when the subtree has none, and a selected session gives its own. One directory copies at once. Several draw the same list control a gate draws, without the typing row. A done run still answers `y`, because the key only reads. A platform or a tool that cannot copy says so in the hint line. Any other typing fills the input bar. The bar names its target from the selection: an open gate on the selected node takes the text as its answer, addressed to that gate, a selected session takes a session message, and anything else sends to the run. When the run is not blocked, the bar says the message queues. One status line holds the last row of the screen, across both panes: the run's state with the first words of its detail, the watch samples the viewer takes, and the latest facts. It is one line, always, cut to the terminal width. The input bar is a line editor: the cursor moves and edits within the text, and up and down recall sent messages. A paste arrives as one unit, newlines kept. A paste over a few lines shows as one atomic token, and sending expands it to the full text. Ctrl-V writes the clipboard image to the run's `attachments/` and inserts the path, so the agent reads the file itself. A file dragged onto the terminal inserts its path the same way. An image or a file is always a path inside message text, never a payload: nothing in the engine, the message, or the adapter changes shape for one. A gate with a shape draws one control, picked from the shape: a list for an enum, a list for an enum beside a string, a checkbox list for an array of enum, yes or no for a boolean, and the input bar with the expected type for anything else. The control draws where the input bar sits, in the same column and at the same width. A list ends with a typing row, so the options and the way to type another answer sit in the same control. Arrows move over the option rows and the typing row. Enter on an option row sends the option under the cursor, or every checked option of a checkbox list. Enter on the typing row sends the typed text alone. Typing on an option row moves to the typing row first. A re-ask with other options starts the cursor and the checkboxes again. The answer goes back as a message addressed to the gate, and an answer from elsewhere closes the control and leaves the typed text for the next target. The engine validates every answer against the shape, and asks the same question again until one fits. Ctrl-C stops the run: a stop kills the steps in flight, records the stop, and the run is done. A done run opens read-only: no input bar, because nothing can react. Its one read-only line draws where the input bar sits, in the same column and at the same width. `q` always goes to the dashboard, and `q` on the dashboard leaves penguin. A view that `pn run` or `pn attach` opened leaves penguin when the run ends while the view is on screen, with the run's outcome as the exit code.
...
16. The input bar holds the bottom of the output column at the transcript width, the status line holds the last row of the screen at its full width, and the status is one line however long the detail.
...
19. Every node the viewer copies from resolves to at least one directory: the directories of the sessions in its subtree, or the folder the run record names. A copy never looks the directory up on disk, so a directory that is gone still copies as text.
