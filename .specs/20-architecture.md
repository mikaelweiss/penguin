...
- **Viewer**: the full-screen terminal UI, one projection of the event files. Bare `pn` opens the dashboard: the live runs with their state, and a needs-you list of every open gate and credential ask across them. `d` reveals a done section under the live runs, every other run on disk with how it ended, and a second `d` hides it. Enter opens a done run read-only. Opening a run shows its view: the run's tree on the left (activities nested under their parents, sessions under the activity that opened them, each with a state glyph), the selected node's transcript on the right, one status line, and the input bar at the bottom. `pn run` opens the view on the new run, and `pn attach` joins one later with its full history. Piped or in cron, penguin prints plain lines instead. Closing a viewer never touches the run.
...
- `~/.penguin/defaults`: one line per role, `<role> <name>`, choosing among installed implementations. Only a role with more than one implementation needs a line. The catalog ships this file, and install copies it with the rest (`30-defaults.md`).
...
`pn run <file> [params]` validates params, creates the run, starts the run process, and attaches a viewer. `--background` starts the run process and prints the run name. `-i` needs a terminal: it claims the run directory first, asks for each non-boolean param the args did not fill with the input field (an enum draws a list), then creates the run. An optional param skips on an empty answer, and a required one asks again. Ctrl-C during entry removes the claimed directory. Without `-i`, a missing param stays an error, and on a terminal the error names the flag. Before the first step the run announces one line: the run name, that the run started, and the default agent adapter's name, or that no agent adapter is installed.
...
- `install`: draw the penguin wordmark, create `~/.penguin/` and `~/.local/state/penguin/runs/`, copy the catalog (`30-defaults.md`) into `~/.penguin/`, which carries the `defaults` file with the role choices, write `penguin-env.d.ts`, then sync the global skills (skills above). The first penguin command runs it.
