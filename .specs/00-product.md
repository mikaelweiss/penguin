...
4. **Definitions are files the user keeps anywhere.** Team workflows, skills, and adapters sit in `<project>/.penguin/` and ship in git. Personal ones sit in `~/.penguin/`. The starter catalog is the `examples/` directory in the penguin package, enabled by path, never copied. penguin scans an ordered list of those catalog directories: the project, then the home, then catalogs enabled in `~/.penguin/catalogs`. Earlier wins. Run state lives under `~/.penguin/`.
...
9. **The engine ships empty, the starter is a catalog.** The engine depends on no adapter and no definition. Install enables the starter catalog (`30-defaults.md`) in `~/.penguin/catalogs` and writes the `defaults` file. It does not copy workflows, adapters, or skills into the home. A home that exists is left alone.
