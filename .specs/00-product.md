...
4. **Definitions are files the user keeps anywhere.** Team workflows, skills, and adapters sit in `<project>/.penguin/` and ship in git. Personal ones sit in `~/.penguin/`. The starter catalog is the `examples/` directory in the engine package, enabled by path, never copied. penguin scans an ordered list of those catalog directories: the project, then the home, then catalogs enabled in `~/.penguin/catalogs`. Earlier wins. Run state lives under `~/.penguin/`.
