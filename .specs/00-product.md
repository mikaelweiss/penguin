...
4. **Definitions are files the user keeps anywhere.** Team workflows, skills, and adapters sit in `<project>/.penguin/` and ship in git. Personal ones sit in `~/.penguin/`. penguin scans an ordered list of those catalog directories, today the project then the home, and runs what it finds. Run state lives under `~/.penguin/`.
