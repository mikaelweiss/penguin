...
A worktree catalog may only add a name. An adapter from a branch is dropped unless it is the sole claim on its role, a skill from a branch is dropped when any other catalog supplies that name, and a file on a branch that refuses to load is skipped rather than thrown, so nothing on a branch changes what the project already resolves.
