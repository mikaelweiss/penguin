...
2. Read the recorded findings as a list, one entry per finding. A heading with its explanation under it is one finding. Only the Blockers and Non-blockers sections hold findings: a What changes table or a How to test list describes the change and is not a finding on either side.
