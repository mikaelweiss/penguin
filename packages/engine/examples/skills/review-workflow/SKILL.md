...
3. Check the penguin checklist:
   ...
   - The file is the happy path: no retry, fix, or gate loop the engine's fault handling already runs. Any `try` around an adapter call handles that one failure differently for a stated reason, and wraps the call in `attempt` so the fault reaches it instead of the gate.
