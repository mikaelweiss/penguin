...
- Write the happy path. Adapter faults are the engine's: no `try` around a call unless this workflow genuinely handles that failure differently, and no re-implementing retry, fix, or gate loops the engine already runs. The call a workflow does handle itself, a best-effort teardown or a freshness probe, runs inside `attempt(() => ...)`, which turns the gate off for that call.
