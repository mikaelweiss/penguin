---
name: wa-review-workflow
description: Reviews a new wa workflow file against its design. Use after a workflow file is written.
---
...
3. Check the wa checklist:
   - `description` is one non-empty line, and `wa list workflows` shows the workflow with the params the design names.
   ...
   - Every skill name the file uses resolves in `wa list skills`.
