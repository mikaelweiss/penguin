...
4. Run `wa list workflows` and confirm the new workflow lists with its description and params. Fix the file until it does.
...
- Import `wa`, `zod`, other workflow files, and shared TypeScript files by relative path. Nothing else.
- Match the idiom of the files in `~/.wa/`: result schemas at the top, pure helpers next, the workflow export last.
