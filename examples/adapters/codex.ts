...
function target(item: Item): string | undefined {
  if (item.type === "command_execution") return flatten(item.command);
  if (item.type === "file_change") {
    return flatten((item.changes ?? []).map((change) => change.path).filter(said).join(", "));
  }
  if (item.type === "mcp_tool_call") {
    return flatten(typeof item.arguments === "string" ? item.arguments : JSON.stringify(item.arguments));
  }
  if (item.type === "web_search") return flatten(item.query);
  if (item.type === "collab_tool_call") {
    return flatten(item.prompt ?? (item.receiver_thread_ids ?? []).join(", "));
  }
  return undefined;
}
...
function flatten(text: string | undefined): string | undefined {
...
export default adapter({
  ...
  name: "codex",
  description:
    "runs skills on the codex CLI. A handle is one conversation: the first turn starts a codex thread, later turns resume it.",
  build: (host) => {
    const threads = new Map<string, string>();
    return {
      async turn(turn: AgentTurn): Promise<AgentTurnResult> {
        const thread = threads.get(turn.session);
        ...
        if (typeof model === "string") argv.push("-c", `model="${model}"`);
        const sandbox = turn.options["sandbox"];
        argv.push("-c", `sandbox_mode="${typeof sandbox === "string" ? sandbox : "workspace-write"}"`);
        const dir =
          turn.schema === undefined ? undefined : fs.mkdtempSync(path.join(os.tmpdir(), "penguin-codex-"));

        try {
          if (turn.schema !== undefined && dir !== undefined) {
            const file = path.join(dir, "schema.json");
            fs.writeFileSync(file, JSON.stringify(strict(turn.schema)));
          ...
          const handle = (line: string): void => {
            ...
            if (said(event.thread_id)) threads.set(turn.session, event.thread_id);
            ...
            if (event.type === "item.completed" && said(item.text)) {
              if (item.type === "agent_message") {
                message = item.text;
                host.emit({ type: "agent", session: turn.session, kind: "text", text: item.text });
              }
              if (item.type === "reasoning") {
                host.emit({ type: "agent", session: turn.session, kind: "thinking", text: item.text });
              }
            }
            if (event.type === "item.completed" && item.type === "error" && said(item.message)) {
              host.emit({ type: "agent", session: turn.session, kind: "text", text: item.message });
            }
          ...
          if (code !== 0) {
            ...
            return {
              ok: false,
              error: tail === "" ? `codex exited with code ${code}` : `codex exited with code ${code}: ${tail}`,
            };
          }
          ...
          return { ok: true, value: clean(value, turn.schema, turn.schema) };
        } finally {
