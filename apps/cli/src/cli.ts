...
async function listAdapters(verbose = false): Promise<number> {
  const list = await installed(process.cwd());
  if (list.length === 0) {
    say(`no adapter file in ${searchedAdapters(process.cwd()).map(short).join(" or ")}`);
    return 0;
  }
  writeEnv(process.cwd(), list);
  say(adapterBlocks(list, verbose));
  return 0;
}

async function listWorkflows(hint = false, verbose = false): Promise<number> {
  const list = await listed(process.cwd());
  if (list.length === 0) {
    say(`no workflow file in ${searchedWorkflows(process.cwd()).map(short).join(" or ")}`);
    if (hint) process.stdout.write(`\n${usage}`);
    return 0;
  }
  say(workflowBlocks(list, verbose));
  if (hint) say("\nrun one with: pn run <workflow> [--param value ...]");
  return 0;
}

function listSkills(verbose = false): number {
  const list = availableSkills(process.cwd());
  if (list.length === 0) {
    say("no skill yet. pn sync-skills links the ones you have");
    return 0;
  }
  say(skillBlocks(list, verbose));
  return 0;
}
...
async function runWorkflow(argv: string[]): Promise<number> {
  ...
  const installedAdapters = await installed(process.cwd());
  writeEnv(process.cwd(), installedAdapters);
  const name = asked
...
async function chooseWorkflow(): Promise<string | undefined> {
  ...
  const picked = await pickOne("which workflow?", choices(list));
...
function sourceOf(target: string): string {
  ...
  const named = locate(target, process.cwd());
  if (named !== undefined) return named;
  const places = searchedWorkflows(process.cwd()).join(" or ");
