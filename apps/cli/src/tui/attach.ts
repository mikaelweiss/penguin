...
export function agentLine(found: AdapterFound[]): string {
  const picked = pick(found, "agent");
...
export function agentLabel(found: AdapterFound[]): string {
  const picked = pick(found, "agent");
  if ("found" in picked) return `agent ${picked.found.name}`;
...
export async function attach(name: string, pid?: number): Promise<number> {
  ...
  const found = await installed(record.cwd);
