...
async function announce(
  ...
  await notifyNeedsYou(run.id, notice.title, notice.body, sound);
}
...
export function useNeedsYou(
  ...
  const latest = useRef({ projects, openId, onOpen, sound });

  useEffect(() => {
    latest.current = { projects, openId, onOpen, sound };
  });
  ...
  useEffect(() => {
    ...
    const { openId: shown, sound: chosen } = latest.current;
    for (const run of newlyBlocked(was, projects)) {
      const notice = needsYouNotice(run);
      if (notice === undefined) continue;
      announce(run, notice, shown, chosen).catch(() => {});
    }
  }, [projects, published]);
