/** The browser tab, the one place to pick a file that is not open yet. */
...
export type SessionTabState = {
  tabs: SessionTabs;
  /** The italic tab. The next preview takes its slot instead of adding one. */
  preview?: string;
};
...
/** Shows a tab in the preview slot, replacing whatever sat there. */
...
/** Pins a tab so the next preview cannot take its slot. Review keeps the preview alive. */
...
/** Closes a tab and hands the active slot to its left neighbour. Review is never removed. */
